...
const allTranslations = {
  ...
  "headers": {
    ...
    "navigationDescription": "Access the main sections of our website",
    "brandName": "Router Park",
    "apiMonitor": "API Monitor",
    "chatgptMirrors": "ChatGPT Mirrors",
    ...
    "announcementTitle": "Join Our Community",
    "announcementDesc": "Scan the QR code below to join our QQ group",
    "qqGroupQR": "QQ Group QR Code"
  },

  // Footer translations
  "footer": {
    "brandName": "AI Navigation",
    "description": {
      "line1": "Discover the best AI tools",
      "line2": "Boost your productivity"
    },
    "products": {
      "title": "Products",
      "features": "Features",
    ...
    "copyright": "© 2025 Xiguapi. All rights reserved.",
    "legal": {
...
// Function to add translations to a JSON file
function addTranslations(filePath) {
  try {
    ...
    // Add translations with English as fallback
    Object.keys(allTranslations).forEach(key => {
      if (!json[key]) {
        json[key] = allTranslations[key];
      }
    });
    ...
    console.log(`✅ Successfully added translations to ${filePath}`);
  } catch (error) {
...
console.log('🚀 Adding all component translations to all language files...\n');

languages.forEach(lang => {
  const filePath = path.join(__dirname, '..', 'messages', `${lang.code}.json`);
  addTranslations(filePath);
});

console.log('\n✨ All done! All component translations have been added to all language files.');
